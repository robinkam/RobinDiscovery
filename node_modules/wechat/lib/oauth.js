 ...
 * Examples:
 * ```
 * var OAuth = require('myWeixinAPI').OAuth;
 ...
 * 获取openid之后，可以调用`myWeixinAPI.API`来获取更多信息
 * Examples:
 * ```
...
OAuth.prototype._getUser = function (openid, accessToken, callback) {
...
OAuth.prototype.getUser = function (openid, callback) {
  var that = this;
  this.getToken(openid, function (err, data) {
    if (err) {
      return callback(err);
    }
    // 没有token数据
    if (!data) {
      var error = new Error('No token for ' + openid + ', please authorize first.');
      error.name = 'NoOAuthTokenError';
      return callback(error);
    }
    var token = AccessToken(data);
    if (token.isValid()) {
      that._getUser(openid, token.data.access_token, callback);
    } else {
      that.refreshAccessToken(token.data.refresh_token, function (err, token) {
        if (err) {
          return callback(err);
        }
        that._getUser(openid, token.access_token, callback);
      });
    }
  });
};
